...
// clean protocol channel. Auth is the deployment's single account password.
...
import { DrawdbClient, ApiError } from "./api.js";
import {
  DATABASES,
  CARDINALITIES,
  CONSTRAINTS,
...
server.registerTool(
  ...
  async ({ diagramId, confirm }) => {
    try {
      if (confirm !== true) {
        return fail(
          new Error("Refusing to delete: pass confirm:true to proceed."),
        );
      }
